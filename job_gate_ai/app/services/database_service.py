...
class DatabaseService:
    ...
    def _get_or_create_user(self, session, user_id: str, email: str = None) -> Optional[User]:
    ...
    def get_or_create_user(self, user_id: str, email: str = None) -> Optional[User]:
        with self.get_session() as session:
            return self._get_or_create_user(session, user_id, email)

    def save_cv_analysis(self, analysis_data: Dict) -> Optional[CVAnalysis]:
        try:
            with self.get_session() as session:
                user = self._get_or_create_user(session, analysis_data["user_id"])
                if not user:
    ...
    def create_builder_session(self, user_id: str, session_data: Dict) -> Optional[BuilderSession]:
        try:
            with self.get_session() as session:
                ...
                existing = session.query(BuilderSession).filter_by(
    ...
    def update_builder_session(self, session_id: str, updates: Dict) -> bool:
        try:
            with self.get_session() as session:
                builder_session = session.query(BuilderSession).filter_by(
    ...
    def create_chatbot_session(self, session_data: Dict) -> Optional[ChatbotSession]:
        try:
            with self.get_session() as session:
                existing = session.query(ChatbotSession).filter_by(
                    session_id=session_data["session_id"]
                ).first()
                if existing:
                    return existing
                ...
                record = ChatbotSession(
                    ...
                    output_language=session_data.get(
                        "output_language", session_data.get("language", "english")
                    ),
