        ...
        action={
          <div className="flex flex-col gap-2 md:flex-row md:items-center">
            <select
              ...
              aria-label={(copy as any).chooseJob ?? (language === "ar" ? "اختر الوظيفة" : "Choose Job Post")}
            >
              <option value="">
                {(copy as any).chooseJobPlaceholder ??
                  (language === "ar" ? "حدد إعلان وظيفي" : "Select a job post")}
              </option>
              {jobPostings.map((job) => (
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-[var(--text-muted)]">
            {language === "ar" ? "مرتب حسب الأحدث" : "Sorted by latest"}
          </span>
        </div>
        ...
        {!selectedJobId ? (
          <div className="mt-4 rounded-xl border border-[var(--panel-border)] p-4 text-sm text-[var(--text-muted)]">
            {(copy as any).chooseJobFirst ??
              (language === "ar"
                ? "اختر إعلان وظيفي أولاً لعرض المتقدمين."
